...
\begin{enumerate}
    ...
    \item Prototüübi täismahuline käivitamine kogu testkogu vastu mõlema prompti versiooniga ja mõlema mudeliga (Claude 4.7 Opus, GPT-5); mudeli väljundi võrdlemine kuldstandardiga ja näitajate (täpsus, saagis, F\textsubscript{1}) arvutamine iga kategooria kohta eraldi.
...
Tulemuste üldistatavuse kontrollimiseks on kavandatud paralleelselt katsetada ka OpenAI \texttt{gpt-5} mudelit, mille tugi on prototüübis realiseeritud. Käesoleva töö pilootuuringus käivitati üksikud katsekatkendid mõlema mudeliga, et veenduda, kas struktureeritud prompti kasu üldistub mudelite vahel; täismahuline mudelivahelise tundlikkuse analüüs jääb jätkukavasse. Claude'i puhul kasutatakse temperatuuri väärtust 0,2 -- madal temperatuur toob kaasa stabiilsemad tulemused, kuid säilitab teatud loovuse, mis on keele- ja stiilihinnangute andmiseks vajalik. GPT-5 puhul kasutatakse mudeli vaikimisi parameetreid, kuna uuemate OpenAI mudelite Chat Completions API ei toeta enam temperatuuri muutmist kõikidel variantidel~\cite{openai_gpt5_2026}.
...
Täismahulises hindamises annoteeritakse iga katkendi puhul probleemid kahes etapis. Esiteks märgistab autor iseseisvalt iga tuvastatud probleemi: probleemse tekstilõigu, kategooria ning lühikese põhjenduse. Teiseks vaatab teine sõltumatu annoteerija (näiteks teise õppekava tudeng või juhendaja) samad katkendid läbi, ning lahkarvamuste korral lepitakse kokku ühises lahenduses. Inter-annoteerija kokkulepet mõõdetakse Coheni $\kappa$~\cite{cohen_kappa_1960} või Krippendorffi $\alpha$~\cite{krippendorff_alpha_2011} mõõdiku abil iga kategooria kohta eraldi.
