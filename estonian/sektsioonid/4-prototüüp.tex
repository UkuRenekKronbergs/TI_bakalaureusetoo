...
Prototüübi realiseerimisel valiti tehnoloogiad järgmiste põhimõtete alusel: tuntud ja levinud tehnoloogiad (et reprodutseeritavus oleks lihtne), minimaalne ülaltvedu (\emph{ingl. overhead}) ja kiire arendustsükkel.
...
\begin{table}[htb!]
    ...
    Backend & Python 3.12 + FastAPI & Anthropic SDK on Pythonis küps; FastAPI annab automaatselt OpenAPI dokumentatsiooni ja valiidimisrakendused. \\
    ...
    LLM-i API & Anthropic Messages API & Claude 3.5 Sonneti tugi, struktureeritud väljundi tugi (\texttt{tools} või JSON-mode). \\
    Skeemivalideerimine & Pydantic + JSON Schema & Backendis valideerib mudeli vastust enne kasutajale saatmist; vea korral palutakse mudelilt parandus. \\
...
Promptid on hoitud eraldi failidena \texttt{prompts/yldine.txt} ja \texttt{prompts/struktureeritud.txt}, mis võimaldab neid muuta ilma koodi puudutamata. Sisenditeks olevad muutujad \texttt{\{peatüki\_tüüp\}} ja \texttt{\{tekst\}} asendatakse Pythoni \texttt{string.Template}'iga, vältides Jinja2-tüüpi malliga kaasnevaid külgmõjusid.

LLM-i päring on tehtud sünkroonse kutsega Anthropici SDK kaudu (vt joonis~\ref{fig:kood-llm}), kasutades parameetrit \texttt{response\_format} JSON-väljundi nõudmiseks. Vastus dekodeeritakse \texttt{json.loads}'iga ja seejärel valideeritakse Pydantic-mudeli vastu. Kui valideerimine ebaõnnestub (mudel andis vigane skeem), tehakse mudelile järelpäring, mis sisaldab esialgset vastust ja konkreetset veatekst kommentaariga \emph{Palun paranda vastus järgmise skeemi järgi}. Praktikas piisas peaaegu kõikidel juhtudel ühest järelpäringust.
...
def kysi_tagasiside(prompt: str, mudel: str = "claude-3-5-sonnet-20241022") -> Vastus:
    klient = Anthropic()
    ...
    tekst = sonum.content[0].text
    return Vastus.model_validate_json(tekst)
\end{minted}
\caption{LLM-i kutsumise lihtsustatud Pythoni-kood. Tegelikus prototüübis lisanduvad logimise, tulekule (\emph{ingl. retry}) ja vea käsitlemise abstraktsioonid.}
\label{fig:kood-llm}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Prototüübi kasutajaliidese mall. Vasakul: tekstisisend ja konfiguratsioon. Paremal: kategoriseeritud tagasiside, kus iga leid on kuvatud eraldi kaardina koos probleemse tsitaadi, kirjelduse, põhjenduse, soovituse ja kindlushinnanguga. Joonisel kasutatud kuva on illustratiivne paigutuse näide.}
...
Süsteemi disainimisel on otseselt arvestatud sellega, et see ei muutuks tudengi jaoks teksti \emph{loomise} vahendiks. Selle tagamiseks rakendati neli täpsustatud meetmed.
...
\textbf{Selge teavitus kasutajaliidesest.} Igas vastuses on tagasiside paneeli päises pidevalt nähtav teade: \emph{See süsteem annab soovituslikku tagasisidet sinu enda kirjutatud tekstile. See ei kirjuta lõputööd sinu eest. Kontrolli iga soovitus üle ja sõnasta vajadusel ümber.}

\textbf{Privaatsus.} Kasutaja sisestatud tekst saadetakse LLM-i API-le ainult pärast seda, kui kasutaja on kinnitanud konkreetses dialoogiaknas, et ta on sellest teadlik. Kohalik backend ei salvesta sisestatud teksti kettal; logitakse ainult anonüümne sündmus (näitajaid: prompti tüüp, peatüki tüüp, leidude arv kategooriate kaupa) hindamise jaoks.
